...
const InProgress = (props) => {
    ...
    useEffect(() => {
        let inprogressData = props.candidate.inprogressData;
        if (inprogressData !== undefined) {
            if (inprogressData?.data?.status === "success") {
                setList(inprogressData.data.data.response);
            }
        }
    }, [props.candidate.inprogressData]);
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Button
                    variant="contained"
                    style={{ backgroundColor: "green", color: "white" }}                // onClick={() => handleReSubmission(params.row.resubmit.id)}
                >
                    InProgress
                </Button>
            ),
