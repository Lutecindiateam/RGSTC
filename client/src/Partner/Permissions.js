...
const permissions = ["access calendar", "access chat", "access fileManager", "scheme management", "ajnjsxisixb", "xnxnidnxid", "ewowniwnid", "eieieuiei"]
    ...
    const permissionElements = permissionNames.map((item) => (
        <Fragment key={item.id}>
            <Checkbox
                ...
                checked={selectedPermission[item]}>
                {item}
            </Checkbox>
        </Fragment>
    ));
...
const AddPermission = ({ onDataUpdate }) => {
    ...
    useEffect(() => {
        onDataUpdate(selectedPermission);
    }, [selectedPermission])
    // useEffect(() => {
